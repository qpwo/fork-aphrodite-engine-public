...
class ChatCompletionRequest(OpenAIBaseModel):
    ...
    passthrough: Passthrough = pydantic.Field(default_factory= lambda: Passthrough(foo=-4, bar=''))
...
class CompletionRequest(OpenAIBaseModel):
    ...
    xtc_threshold: Optional[float] = 0.1
    xtc_probability: Optional[float] = 0.0
    passthrough: Passthrough = pydantic.Field(default_factory= lambda:Passthrough(foo=-5, bar=''))
    dry_multiplier: Optional[float] = 0
    dry_base: Optional[float] = 1.75
    dry_allowed_length: Optional[int] = 2
    dry_sequence_breakers: Optional[List[str]] = Field(
...
class KAIGenerationInputSchema(BaseModel):
    ...
    xtc_threshold: Optional[float] = 0.1
    xtc_probability: Optional[float] = 0.0
    passthrough: Passthrough = pydantic.Field(default_factory= lambda:Passthrough(foo=-6, bar=''))
